...
# TWD203_currency_interpolation.R

library(xts)
library(xtsExtra)

# read in the indicator matirx 
indicator <-  read.csv('TWD203_currency_indicator.csv',stringsAsFactors = FALSE)
...
write.csv(as.data.frame(data_after), file = "TWD203_currency_interpolated.csv",row.names=TRUE)
write.csv(result, file = "TWD203_currency_result_table.csv",row.names=TRUE)
