...
index <- read.csv('TWD203_index.csv',stringsAsFactors = FALSE)
...
jpeg(file = " TWD203 index LOESS plot %d.jpeg",quality=100,width = 1200, height = 800,units = 'px', pointsize = 12)

indi <- rep(0,nrow(t)) # initialize the indicator vector

for (i in 1:ncol(t))
{
  a <- tsoutliers(t[,i],name = rate_name[i],span=span, percentile = c(0.15,0.85),k=3,range=c(-0.9,1.1))
...
jpeg(file = "TWD203 Index.jpeg",quality=100,width = 1200,height = 800,units = 'px', pointsize = 12)
...
plot.xts(t, screens = factor(1, 1), panel = custom.panel, auto.legend = TRUE, main = "TWD203 index")
dev.off()
...
write.csv(data, file = "TWD203_index_outlier.csv",row.names=TRUE)
...
jpeg(file = " TWD203 index check plot %d.jpeg",quality=100,width = 1200, height = 800,units = 'px', pointsize = 12)
par(mfrow=c(3,3))
