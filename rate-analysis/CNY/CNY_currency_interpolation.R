# CNY_currency_interpolation.R

library(xts)
library(xtsExtra)

# read in the indicator matirx 
indicator <-  read.csv('CNY_currency_indicator.csv',stringsAsFactors = FALSE)
...
write.csv(as.data.frame(data_after), file = "CNY_currency_interpolated.csv",row.names=TRUE)
write.csv(result, file = "CNY_currency_result_table.csv",row.names=TRUE)
