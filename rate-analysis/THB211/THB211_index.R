...
index <- read.csv("THB211_indedx.csv",stringsAsFactors = FALSE)
...
jpeg(file = " THB211  index LOESS plot %d.jpeg",quality=100,width = 1200, height = 800,units = 'px', pointsize = 12)

indi <- rep(0,nrow(t)) # initialize the indicator vector

for (i in 1:ncol(t))
{
  a <- tsoutliers(t[,i],name = rate_name[i],span=span, percentile = c(0.22,0.88),k=3,range=c(-0.9,1))
...
jpeg(file = "THB211 index.jpeg",quality=100,width = 1200,height = 800,units = 'px', pointsize = 12)
...
plot.xts(t, screens = factor(1, 1), panel = custom.panel, auto.legend = TRUE, main = "THB211 index")
dev.off()

data <- cbind(indi,t)
data <- data.frame(date=index(data), coredata(data))
colnames(data)[2] <- "indicator" 
write.csv(data, file = "THB211_index_outlier.csv",row.names=TRUE)
...
jpeg(file = " THB211 index check plot %d.jpeg",quality=100,width = 1200, height = 800,units = 'px', pointsize = 12)
par(mfrow=c(3,3))
