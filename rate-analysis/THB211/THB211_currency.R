...
index <- read.csv("THB211_currency.csv",stringsAsFactors = FALSE)
...
jpeg(file = " THB211  currency LOESS plot %d.jpeg",quality=100,width = 1200, height = 800,units = 'px', pointsize = 12)

indi <- rep(0,nrow(t)) # initialize the indicator vector

for (i in 1:ncol(t))
{
  a <- tsoutliers(t[,i],name = rate_name[i],span=span, percentile = c(0.15,0.85),k=2,range=c(-0.9,1))
...
jpeg(file = "THB211 currency.jpeg",quality=100,width = 1200,height = 800,units = 'px', pointsize = 12)

# try to add points to show outlier
# index(indi[indi == 1]) get outlier
custom.panel <- function(index,x,...) {
  default.panel(index,x,...)
  abline(v=index(indi[indi == 1]),col="red",lwd=0.7)
  usr <- par( "usr" )
  text( usr[ 2 ], usr[ 4 ], paste("number of outliers: ",num,"\n","ratio: ",format(per,digits=4)),  adj = c( 1, 1 ), col = "blue" )

}

plot.xts(t, screens = factor(1, 1), panel = custom.panel, auto.legend = TRUE, main = "THB211 currency", xlab="day",ylab="%")
dev.off()

data <- cbind(indi,t)
data <- data.frame(date=index(data), coredata(data))
colnames(data)[2] <- "indicator" 
write.csv(data, file = "THB211_currency_outlier.csv",row.names=TRUE)
...
jpeg(file = " THB211 currency check plot %d.jpeg",quality=100,width = 800, height = 600,units = 'px', pointsize = 12)
par(mfrow=c(2,2))
